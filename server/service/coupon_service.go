...
func VerifyCoupon(order *model.Orders) (model.Coupon, error) {
	...
	//判断使用次数
	orderArr, err := CommonSqlFind[model.Orders, string, []model.Orders](model.Orders{}, "user_id = "+strconv.FormatInt(order.UserID, 10)+" AND coupon_id = "+strconv.FormatInt(c.ID, 10))
	if err != nil {
		return model.Coupon{}, errors.New("优惠码错误")
	}
